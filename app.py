...
# --- Tab 1: Overview ---
with overview_tab:
    st.image("https://upload.wikimedia.org/wikipedia/en/thumb/f/fd/F1.svg/1200px-F1.svg.png", width=120)
...
with graphs_tab:
    ...
    if option == "Q1 Lap Time Distribution":
        ...
        st.pyplot(fig)
        st.caption("Most drivers clock lap times between 78–100 seconds. The slight right skew indicates a few slower performances.")
        ...
        st.pyplot(fig)
        st.caption("Drivers starting in front rows tend to finish better. Back-grid drivers show wider position variation.")
        ...
        ax.set_ylabel("Number of Drivers")
        st.pyplot(fig)
        st.caption("Top 5 starters maintain or improve position. Others show more volatility in performance.")
        ...
        st.pyplot(fig)
        st.caption("Top 10 finishers earn consistent points. 1st place drivers typically score the most.")
        ...
        ax.set_ylabel("Final Race Position")
        st.pyplot(fig)
        st.caption("Fastest lap doesn't guarantee a win — speed isn't everything in F1.")
        ...
        st.pyplot(fig)
        st.caption("Driver 1 shows best average, while others vary more in performance.")
